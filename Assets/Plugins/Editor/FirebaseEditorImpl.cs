...
public class FirebaseEditorImpl : QueryEditorImpl, IFirebase {
	...
	//[DllImport ("FirebaseProxy")]
	//private static extern void _FirebaseCreateUser (IntPtr firebase, string email, string password, long callback);
	#endregion
	...
	{
		Debug.Log("Firebbase 'CreateUse' method is not implemented in editor.");
		//long newnumber = activeCallEntry != null ? (activeCallEntry.Instance + 1) % long.MaxValue - 1 : 0;
		//activeCallEntry = new AuthCallInstanceEntry() { Instance = newnumber, OnSuccess = null, OnError = onError};
		//_FirebaseCreateUser(GetEditorObject(), email, password, activeCallEntry.Instance);
	}
