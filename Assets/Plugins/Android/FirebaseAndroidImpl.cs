...
internal class FirebaseAndroidImpl : QueryAndroidImpl, IFirebase
{
	...
	public void CreateUser (string email, string password, Action onSuccess, Action<FirebaseError> onError)
	{
		GetJavaObject().Call("createUser", email, password, new ResultListener(onSuccess, onError));
	}
	#endregion
	...
	class ResultListener : AndroidJavaProxy
	{
		...
		Action<FirebaseError> onErrorHandler;

		public ResultListener (Action onSuccess, Action<FirebaseError> onError)
			: base("com.firebase.client.Firebase$ResultHandler")
		{
		...
		void onError(AndroidJavaObject firebaseError) {
			FirebaseErrorAndroidImpl errorImpl = new FirebaseErrorAndroidImpl (firebaseError);
			if (onErrorHandler != null) {
				onErrorHandler(new FirebaseError( errorImpl.Code, errorImpl.Message, errorImpl.Details));
			}
		}
